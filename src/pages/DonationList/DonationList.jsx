                    ...
                    {allDonation.map((donation) => {
                        return (
                            ...
                            >
                                ...
                                />
                                <h2 className="text-2xl text-gray-900 font-bold dark:text-white">{donation.petName}</h2>
                                <p className="text-gray-600 mt-2 dark:text-gray-400">{donation.shortDescription}</p>

                                <div className="mt-3 flex justify-between items-center">
                                    <div>
                                        <p className="text-sm text-gray-600 dark:text-gray-400">Max Donation: ${donation.maxDonationAmount}</p>
                                    </div>
                                    <Link
                                        to={`/donation/${donation._id}`}
                                        className={`bg-blue-500 px-6 py-3 hover:bg-blue-700 text-white rounded-md text-sm font-semibold ${donation?.pause ? "cursor-not-allowed opacity-50" : ""} dark:bg-blue-600 dark:hover:bg-blue-500`}
                                    >
                                        View Details
                                    </Link>
                                </div>
